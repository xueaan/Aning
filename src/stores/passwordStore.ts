...
const api = {
  ...
  createEntry: (entry: PasswordEntry) =>
    invoke<number>('create_password_entry', { entry }),
  ...
  getDecryptedPassword: (entryId: number) =>
    invoke<string>('get_decrypted_password', { entryId }),
    ...
    persist(
      (set, get) => ({
        ...
        loadCategories: async () => {
          ...
          const categories = await withErrorHandling(
            api.getCategories,
            setError
          );
        ...
        updateEntry: async (id, entry) => {
          ...
          if (result !== null) {
            await loadEntries();
            set({ isEditing: false, editingEntry: undefined });
          }
        },
