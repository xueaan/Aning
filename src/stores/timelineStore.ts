  ...
  devtools(
    (set, get) => ({
      ...
      getTodayEntries: () => {
        ...
        return get().entries.filter((entry) => entry.created_at.startsWith(today));
      },

      getEntriesByDate: (date) => {
        return get().entries.filter((entry) => entry.created_at.startsWith(date));
      },

      getUnconvertedEntries: () => {
        return get().entries.filter((entry) => !entry.converted_to_note_id);
      },
    }),
    {
      name: 'timeline-store',
