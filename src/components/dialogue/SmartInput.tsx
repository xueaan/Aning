...
export const SmartInput: React.FC<SmartInputProps> = ({
  ...
  // 检测@提及
  const detectMentionInInput = useCallback((text: string, cursorPos?: number) => {
  ...
  }, [showSuggestions, suggestions, selectedSuggestionIndex, handleSelectSuggestion, disabled, onSend, isComposing]);
  ...
  const handlePaste = useCallback(async (e: React.ClipboardEvent) => {
    ...
    if (text && text.length > 100) { // 长于100字符的文本
      try {
        // 创建一个临时上下文项
  ...
  return (
    <div ref={containerRef} 
      ...
      {activeContextsList.length > 0 && (
        <div className="mb-3">
          <ContextTags contexts={activeContextsList} onRemove={removeContext}
          stats={stats}
          />
        </div>
      )}
      ...
      >
        ...
        <div className="absolute bottom-2 right-2 flex items-center gap-1.5">
          {/* 字符计数 */}
          {showCharCount && (
            <span className={`text-xs px-2 py-1 rounded ${remainingChars < 0 ? 'theme-text-error' :
                remainingChars <= 50 ? 'theme-warning' : 'theme-text-secondary'
              }`}>
              {remainingChars}
            </span>
          )}
