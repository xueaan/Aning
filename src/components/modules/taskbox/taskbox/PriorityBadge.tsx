...
export const PriorityBadge: React.FC<PriorityBadgeProps> = ({ 
  ...
  if (showText) {
    return (
      <span className={`inline-flex items-center font-medium rounded ${config.bgColorClass} ${config.textColorClass} ${config.borderColorClass} border ${getTextSizeClass()}`}>
        <div className={`${getSizeClass()} rounded-full flex-shrink-0`}
  ...
  return (
    <div className={`${getSizeClass()} rounded-full flex-shrink-0`}
      style={{ backgroundColor: config.style.backgroundColor }} title={`优先级: ${config.text}`}
    />
  );
};
