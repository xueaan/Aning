  ...
  <div className="flex-1 overflow-auto p-3 md:p-4 lg:p-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-3 md:gap-4 lg:gap-6 auto-rows-fr">
      {columns.map(column => {
        ...
        return (
          <div key={column.id}
            className={`flex flex-col bg-bg-secondary border border-border-primary rounded-lg transition-all min-h-[300px] ${isDragOver ? 'ring-2 ring-accent ring-opacity-50 bg-accent/5' : ''
            }`}
      ...
      <div className="flex-shrink-0 px-3 md:px-4 py-3 border-b border-border-primary">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <button onClick={() => setShowQuickAdd(column.id)}
    ...
    <div className="flex-1 p-2 md:p-3 space-y-2 overflow-y-auto">
      ...
      {showQuickAdd === column.id && (
        <div className="mb-3">
          <SimpleTaskQuickAdd onSubmit={handleQuickAdd} onCancel={() => setShowQuickAdd(null)}
            defaultStatus={column.id} placeholder={`在${column.title}中添加...`}
            compact />
        </div>
      )}
      ...
      {columnTasks.map(task => (
        <div key={task.id} draggable={true}
                    onDragStart={(e) => handleDragStart(e, task)}
      onDragEnd={handleDragEnd}
            className={`cursor-move transition-opacity ${draggedTask?.id === task.id ? 'opacity-50' : ''
        }`}
                  >
      <TaskItem task={task} projects={projects}
      onUpdate={(updates) => onTaskUpdate(task.id!, updates)} onDelete={() => onTaskDelete(task.id!)}
      compact={true} showProject={true}
                    />
    </div>
                ))}
