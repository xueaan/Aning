...
const colors = [
  {
    name: 'blue',
    bg: 'theme-bg-accent/30',
    border: 'theme-border-accent',
    header: 'theme-bg-accent/50',
  },
  {
    name: 'green',
    bg: 'theme-bg-success/30',
    border: 'theme-border-accent',
    header: 'theme-bg-success/50',
  },
  {
    name: 'purple',
    bg: 'theme-bg-secondary/40',
    border: 'theme-border-accent',
    header: 'theme-bg-secondary/60',
  },
  {
    name: 'yellow',
    bg: 'theme-bg-warning/30',
    border: 'theme-border-accent',
    header: 'theme-bg-warning/50',
  },
  {
    name: 'red',
    bg: 'theme-bg-error/30',
    border: 'theme-border-error',
    header: 'theme-bg-error/50',
  },
  {
    name: 'gray',
    bg: 'theme-bg-secondary/30',
    border: 'theme-border',
    header: 'theme-bg-secondary/50',
  },
];
...
export const GroupNode: React.FC<NodeProps> = ({ data, id }) => {
  ...
  const toggleCollapse = () => {
    ...
    if (currentBoard) {
      ...
      const updatedEdges = currentBoard.edges.map((edge) => {
        const isRelatedToHiddenNode =
          data.nodeIds?.includes(edge.source) || data.nodeIds?.includes(edge.target);
        return {
          ...edge,
  ...
  const handleDelete = () => {
    if (deleteNode) {
      ...
      if (currentBoard && data.nodeIds) {
        ...
        const updatedNodes = currentBoard.nodes
          .map((node) => {
            if (data.nodeIds.includes(node.id)) {
              // 将相对坐标转换回绝对坐标
  ...
  return (
    <div className="group-node relative group" style={{ width: '100%', height: '100%' }}>
      ...
      <div
        className={`rounded-lg ${currentColor.bg} ${currentColor.border} border-2 border-dashed`}
        style={{
      ...
      >
        <div
          className={`flex items-center justify-between px-3 py-2 ${currentColor.header} rounded-t-md cursor-pointer`}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs theme-text-secondary">({nodeCount} 节点)</span>
          </div>
        </div>
        ...
        {!isCollapsed && (
          <div className="p-4">
            ...
            <div className="text-xs theme-text-secondary text-center mt-4">拖动节点到此区域</div>
          </div>
        )}
      </div>
