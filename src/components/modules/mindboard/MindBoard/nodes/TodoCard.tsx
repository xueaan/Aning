...
const colors = [
  {
    ...
    header: 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-700',
  ...
  },
  {
    ...
    header: 'bg-orange-50 dark:bg-orange-900/30 border-orange-200 dark:border-orange-700',
  ...
  },
  {
    ...
    header: 'theme-bg-secondary/30 theme-border',
    progress: 'theme-bg-secondary',
  },
];
...
export const TodoCard: React.FC<NodeProps> = ({ data, id }) => {
    ...
    (newTodos: TodoItem[]) => {
      if (currentBoard) {
        const updatedNodes = currentBoard.nodes.map((node) =>
          node.id === id ? { ...node, data: { ...node.data, todos: newTodos, colorIndex } } : node
        );
  ...
  return (
    <div className="todo-card-node relative group">
      ...
      <div
        className={`theme-bg-primary/80 backdrop-blur-sm rounded-lg shadow-lg border-2 ${currentColor.border} min-w-[280px] max-w-[400px]`}
      >
        <div className={`px-3 py-2 ${currentColor.header} border-b`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <CheckSquare className="w-4 h-4 theme-text-secondary" />
              <span className="text-sm font-medium theme-text-primary">任务卡片</span>
            </div>
            {totalCount > 0 && (
              <div className="flex items-center gap-2">
                ...
                <div className="w-16 h-2 theme-bg-secondary/50 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${currentColor.progress} transition-all duration-300`}
            ...
            {todos.map((todo) => (
              ...
              >
                <button
                  ...
                  className="flex-shrink-0 theme-text-accent hover:theme-text-accent/80"
                >
                  {todo.completed ? (
                ...
                <span
                  className={`flex-1 text-sm ${
                    todo.completed ? 'line-through theme-text-secondary/70' : 'theme-text-primary'
                  }`}
                >
                  {todo.text}
                </span>
