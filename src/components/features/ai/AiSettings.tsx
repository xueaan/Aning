...
import { AI_PROVIDERS, AiProviderType, validateApiKey, type AiConfig } from '@/types/aiConfig';
import { testAiConnection, getAiProviderStatus } from '@/utils/aiUtils';

export const Ais: React.FC = () => {
  const { aiConfig, setAiConfig, setCurrentAiProvider } = useAppStore();
  ...
  const handleTestConnection = async () => {
    if (
      ...
      !validateApiKey(selectedProvider, currentConfig.apiKey, currentConfig.baseURL)
    ) {
      setTestResult({
        success: false,
      ...
      <div className="flex gap-2">
        {Object.values(AI_PROVIDERS).map((provider) => (
          <button
            ...
            className={`flex-1 p-3 rounded-lg text-sm transition-colors flex items-center justify-center gap-2 ${
      ...
      <div className="p-4 rounded-xl feather-glass-deco">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              type="text"
              value={currentConfig.baseURL || currentProvider.baseURL}
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm theme-text-accent theme-bg-accent/10 px-2 py-1 rounded">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm theme-text-success status-success/10 px-2 py-1 rounded">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={!currentConfig.apiKey || isTestingConnection}
              className={`px-4 py-2 text-sm rounded-lg theme-text-accent transition-colors ${
                !currentConfig.apiKey || isTestingConnection
                  ? 'opacity-50 cursor-not-allowed'
                  : 'hover:theme-text-primary feather-glass-deco'
              }`}
            >
            ...
            <div className="flex-1">
              ...
              <p
                className={`text-sm mt-1 ${
                  testResult.success ? 'theme-text-secondary' : 'text-red-400'
                }`}
              >
                {testResult.message}
              </p>
