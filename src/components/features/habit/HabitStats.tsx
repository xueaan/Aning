...
import { CheckCircle2, Calendar, Target, TrendingUp, Flame, Star, Crown, Trophy } from 'lucide-react';
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          {trend && (
            <div className={`flex items-center gap-1 mt-2 text-xs ${trend.isPositive ? 'theme-text-success' : 'theme-text-error'
              }`}>
              <TrendingUp 
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-text-muted">{habit.frequency === 'daily' ? '每日' : '每周'}</p>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      {unlocked && (
        ...
        >
          <Crown size={12} 
            className="theme-text-on-accent" />
        </motion.div>
      )}
    </motion.div>
  );
...
export const HabitStats: React.FC = () => {
  ...
  const advancedStats = useMemo(() => {
    ...
    // 完美天数（所有习惯都完成的天数）
    const perfectDays = records.reduce((acc, record) => {
        ...
        <div className="space-y-4">
          <div className="flex items-end justify-between h-32 px-4">
            {[...Array(7)].map((_, i) => {
              ...
              const completionRate = activeHabitsCount > 0 ? (dayRecords.length / activeHabitsCount) * 100 : 0;
