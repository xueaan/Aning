...
const getIconComponent = (title: string, themeColors: any) => {
  const iconMap: Record<string, JSX.Element> = {
    '文本': <Type size={16} color={themeColors.text} />,
    ...
    '任务列表': <CheckSquare size={16} color={themeColors.text} />,
    '引用': <Quote size={16} color={themeColors.text} />,
    '代码块': <Code size={16} color={themeColors.text} />,
    '分割线': <Minus size={16} color={themeColors.text} />,
      ...
      {rows.map((row, rowIndex) => (
        ...
        >
          {row.map((item: any, itemIndex: number) => (
            <div key={itemIndex} onClick={() => onCommand(item)}
              style={{
                width: '40px',
                height: '40px',
                cursor: 'pointer',
  ...
  return () => {
    ...
    return {
      onStart: (props: any) => {
        ...
        // 时光记毛玻璃主题配色
        const themeColors = {
          background: isDarkMode ? 'rgba(20, 20, 20, 0.05)' : 'rgba(255, 255, 255, 0.02)',
          ...
          backgroundHover: isDarkMode ? 'rgba(25, 25, 25, 0.08)' : 'rgba(255, 255, 255, 0.08)',
          shadow: isDarkMode ? '0 8px 32px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.06)' : '0 8px 32px rgba(0, 0, 0, 0.06), inset 0 1px 0 rgba(255, 255, 255, 0.2)'
        };
        ...
        popup.style.setProperty('background', themeColors.background, 'important');
        popup.style.setProperty('backdrop-filter', 'blur(20px) saturate(180%)', 'important');
        popup.style.setProperty('-webkit-backdrop-filter', 'blur(20px) saturate(180%)', 'important');
        ...
        root.render(
          <CommandPanel items={items} onCommand={(item) => {
              if (props.command) {
                props.command(item);
              }
            }}
