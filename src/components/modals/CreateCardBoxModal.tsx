...
interface CreateCardBoxModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const CreateCardBoxModal: React.FC<CreateCardBoxModalProps> = ({ isOpen, onClose }) => {
  const [name, setName] = useState('');
  const [icon, setIcon] = useState('Box');
  const [isLoading, setIsLoading] = useState(false);

  const { createBox, selectBox } = useCardBoxStore();
    ...
    <div className="feather-glass-modal-backdrop" onClick={handleClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b theme-border-primary">
          <h2 className="text-lg font-medium theme-text-primary">创建笔记盒</h2>
          <button
            onClick={handleClose}
