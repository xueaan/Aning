...
import { useCardBoxStore, type CardBox } from '@/stores';
import { IconPicker } from '@/components/common/IconPicker';

interface EditCardBoxModalProps {
  isOpen: boolean;
  onClose: () => void;
  cardBox?: CardBox;
}

export const EditCardBoxModal: React.FC<EditCardBoxModalProps> = ({ isOpen, onClose, cardBox }) => {
  const [name, setName] = useState('');
  const [icon, setIcon] = useState('Box');
  const [isLoading, setIsLoading] = useState(false);

  const { updateBox } = useCardBoxStore();

  // 当笔记盒数据变化时更新表单
  useEffect(() => {
    if (cardBox) {
      setName(cardBox.name);
      setIcon(cardBox.icon || 'Box');
    }
  }, [cardBox]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cardBox || !name.trim()) return;

    setIsLoading(true);
    try {
      await updateBox(cardBox.id, {
        name: name.trim(),
        description: undefined, // 不需要描述
        color: cardBox.color || '#3B82F6', // 保持原有颜色
        icon,
      });
      handleClose();
    } catch (error) {
      console.error('更新笔记盒失败:', error);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="feather-glass-modal-backdrop" onClick={handleClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b theme-border-primary">
          <h2 className="text-lg font-medium theme-text-primary">编辑笔记盒</h2>
          <button
            onClick={handleClose}
